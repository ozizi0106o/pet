...
(function (root, window, document, factory, undefined) {
  ...
  var TavoCalendar = function TavoCalendar(container_q, user_options) {
    var moment = window.moment || user_options.moment;
    ...
    if (container_q instanceof Element) {
      ...
      var wrapper_el = document.querySelector(container_q);
    ...
    this.elements.wrapper.classList.add(CLASS_CALENDAR); // Extend defaults with user preference

    var config = Object.assign({}, options_default, user_options);
    this.state = {
  ...
  TavoCalendar.prototype.mount = function () {
    ...
    var calendar_info_el, calendar_code_el; // Info

    var calendar_reset_el, calendar_select_date_el, calendar_select_date_start_el, calendar_select_date_end_el; // Code Header

    var calendar_header_el, calendar_month_el, calendar_nav_prev_el, calendar_nav_next_el; // Code Days

    var calendar_week_names_el, calendar_days_el; //Calendar info
    ...
    var moment_copy = this.moment.clone();
    moment_copy.startOf('month'); // Not always start of the month matches start of the week

    var offset = moment_copy.isoWeekday() % (7 + this.locale_data.firstDayOfWeek());
    ...
    var _loop = function _loop() {
      ...
      var that = _this;
      var date = moment_copy.format(that.config.format);
      day_wrapper_el.addEventListener('click', function (ev) {
        that.dayClick(date, ev.currentTarget);
      }, true);
      day_el.textContent = d; //ABSOLUTE RELATIONS
      ...
      if (moment_copy.isSame(moment(_this.state.date, _this.config.format), "day")) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_REL_TODAY;
      } else if (moment_copy.isBefore(moment(_this.state.date, _this.config.format), "day")) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_REL_PAST;
      } else if (moment_copy.isAfter(moment(_this.state.date, _this.config.format), "day")) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_REL_FUTURE;
      } //MEANINGFUL MARKERS


      if (_this.state.highlight.indexOf(moment_copy.format(_this.config.format)) > -1) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_HIGHTLIGHT;
      }

      if (_this.config.highlight_saturday && moment_copy.isoWeekday() === 6) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_HIGHTLIGHT;
      }

      if (_this.config.highlight_sunday && moment_copy.isoWeekday() === 7) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_HIGHTLIGHT;
      }

      if (_this.state.blacklist.indexOf(moment_copy.format(_this.config.format)) > -1) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_OFF + " " + CLASS_CALENDAR_DAY_LOCK;
      }

      if (_this.state.selected.indexOf(moment_copy.format(_this.config.format)) > -1) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_SELECT;
      } //Range select


      if (_this.state.date_start && moment_copy.isSame(_this.state.date_start, 'day')) {
      ...
      if (_this.state.date_start && !_this.state.date_end && moment_copy.isBefore(moment(_this.state.date_start, _this.config.format), "day")) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_LOCK;
      }

      if (_this.state.date_end && moment_copy.isSame(_this.state.date_end, 'day')) {
      ...
      if (_this.state.date_start && _this.state.date_end && moment_copy.isBetween(moment(_this.state.date_start, _this.config.format), moment(_this.state.date_end, _this.config.format), 'day')) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_SELECT + " " + CLASS_CALENDAR_DAY_RANGE_SELECT;
      } //Lock days


      if (moment_copy.isBefore(moment(_this.state.date, _this.config.format), "day") && !_this.config.past_select || moment_copy.isAfter(moment(_this.state.date, _this.config.format), "day") && !_this.config.future_select) {
        day_wrapper_el.className = day_wrapper_el.className + " " + CLASS_CALENDAR_DAY_LOCK;
      }
  ...
  TavoCalendar.prototype.dayClick = function (date, day_el) {
    if (this.config.frozen) return; //Day lock
    ...
    if (this.config.range_select) {
      ...
      if (this.config.multi_select) {
        if (this.state.selected.indexOf(date) > -1) {
          this.state.selected = this.state.selected.filter(function (date_selected) {
