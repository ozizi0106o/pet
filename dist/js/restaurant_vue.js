...
Vue.component("booking", {
  ...
  computed: {
    ...
    filterCataToPets: function filterCataToPets() {
      return this.foods.filter(function (food) {
        return food.cata === "petsFood";
      });
    },
    filterCataToHuman: function filterCataToHuman() {
      return this.foods.filter(function (food) {
...
Vue.component("customPetsFood", {
  ...
  template: "#addToMenu",
  methods: {
    removeItem: function removeItem(fid, menu) {
...
var vm = new Vue({
  ...
  data: {
    ...
    petFoodType: ["沙拉", "鮮食"],
    // 寵物現有的食物類別
    petCustomFoodSelection: [],
    // 被加入的客製寵食
    allFoodSelection: [],
    // 所有被加入的食物
    isLightBoxOpen: false,
    // 食物燈箱啟閉
    isBookingBoxOpen: false,
    // 預約燈箱啟閉
    isDogOut: false,
    forLightBoxInfo: [],
    allFoodMenu: [{
  ...
  methods: {
    choosePetCustomFood: function choosePetCustomFood(food) {
    ...
    // 移除菜單
    remove: function remove(fid, menu) {
      if (menu.count > 1) {
        menu.count -= 1;
      } else {
        var yes = confirm("確定移除?");
    ...
    getLightBoxOpen: function getLightBoxOpen(food) {
      this.isLightBoxOpen = true;
      this.forLightBoxInfo = this.allFoodMenu.find(function (item) {
        return item === food;
      });
    }
  },
  computed: {
    ...
    // 分類鮮食
    filterTypeToFresh: function filterTypeToFresh() {
      return this.allFoodMenu.filter(function (food) {
        return food.type === "鮮食";
      });
    },
    // 分類沙拉
    filterTypeToSalad: function filterTypeToSalad() {
      return this.allFoodMenu.filter(function (food) {
        return food.type === "沙拉";
      });
    },
    // 分類美式
    filterTypeToUS: function filterTypeToUS() {
      return this.allFoodMenu.filter(function (food) {
        return food.type === "美式";
      });
    },
    // 分類義式
    filterTypeToItaly: function filterTypeToItaly() {
      return this.allFoodMenu.filter(function (food) {
        return food.type === "義式";
      });
    },
    // 分類客製寵食
    filterCataToPetsCustom: function filterCataToPetsCustom() {
      return this.allFoodMenu.filter(function (food) {
        return food.cata === "petsCustom";
      });
    },
    filterMenuToPets: function filterMenuToPets() {
      return this.allFoodSelection.filter(function (food) {
        return food.cata === "petsFood";
      });
    },
    filterMenuToHuman: function filterMenuToHuman() {
      return this.allFoodSelection.filter(function (food) {
        return food.cata === "humanFood";
      });
    }
